...
class dados_rel_contrapartida_model extends CI_Model{
	...
	public function insere_dados_rel($options){
		$this->db->insert('dados_rel_contrapartida', $options);
	}
	...
	if($this->session->userdata('nivel') != 2)
		...
		return $this->db->get('dados_rel_contrapartida')->result();
	}
	
	public function get_by_id($id){
		$this->db->where('id_rel', $id);
		return $this->db->get('dados_rel_contrapartida')->row(0);
	}

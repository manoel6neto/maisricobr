...
class usuario_cnpj extends CI_Model {
    ...
    public function get_all_by_subgestor($id_usuario) {
        ...
        $this->db->distinct();
        if ($this->session->userdata('nivel') == 7 || $this->session->userdata('nivel') == 8) {
            $this->db->where('usuario_cnpj.id_usuario', $this->usuario_subgestor_model->get_by_usuario($id_usuario)->id_gestor);
        } else {
            $this->db->where('usuario_cnpj.id_usuario', $id_usuario);
        }
    ...
    public function get_cnpjs_vinculados_vendedores_cnpj() {
        $this->db->distinct();
        $this->db->select('cnpj_vinculado as cnpj');

        return $this->db->get('cnpj_vendedores')->result();
    }

    public function drop_cnpjs_vinculados_vendedores($cnpj) {
        try {
            $this->db->where('cnpj_vinculado', $cnpj);
    ...
    public function get_dados_envia_email($cnpj) {
        ...
        $this->db->distinct();
        $this->db->select('nome, email, id_usuario');

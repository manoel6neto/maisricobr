...
class banco_proposta_model extends CI_Model {
    ...
    public function busca_programas_pareceres($filtros, $busca_cnpj_proponente = false, $filtro_proponente = array(), $geral = FALSE) {
        ...
        if ((!empty($cnpjs) && $this->session->userdata('nivel') != 1) || $this->session->userdata('nivel') == 1 || $geral == 1) {
            $this->db->distinct('banco_proposta.codigo_siconv');
            ...
            if ($this->session->userdata('nivel') != 1 && $geral == false) {
                $filtroPesquisa = "";
                if ($filtros['pesquisa'] != "") {
                ...
                if ($filtros['pesquisa'] != "") {
                    ...
                    $filtroPesquisa .= " OR banco_proposta.tipo LIKE '%{$filtros['pesquisa']}%'";
                    $filtroPesquisa .= " OR banco_proposta.situacao LIKE '%{$filtros['pesquisa']}%'";
                    $filtroPesquisa .= " OR banco_proposta.proponente LIKE '%{$filtros['pesquisa']}%'";
                    $filtroPesquisa .= " OR banco_proposta.codigo_siconv LIKE '%{$filtros['pesquisa']}%')";
    ...
    public function retorna_status_aprovado() {
        return array(
            ...
            strtoupper('Prestação de Contas Rejeitada'),
            strtoupper('Proposta/Plano de Trabalho Aprovados (Cronogramas pendente de Ajustes)'),
            ...
            strtoupper('Adimplente no SIAFI'),
            strtoupper('Inadimplencia Suspensa no SIAFI'),
            strtoupper('Excluído no SIAFI')
        );
    }

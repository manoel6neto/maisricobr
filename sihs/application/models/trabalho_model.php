...
//        $query = $this->db->query("SELECT pv.* from programas_vigencia AS pv JOIN (orgaos AS og) ON (og.nome = pv.orgao) WHERE og.codigo = '{$orgao}' AND pv.qualificacao LIKE '%Proposta Voluntária%' AND pv.atende LIKE '%municipal%' AND (pv.estados LIKE '%todos%' OR pv.estados LIKE '%{$estado[0]->municipio_uf_sigla}%')");
        if ($this->session->userdata('sistema') == 'M') {
            $query = $this->db->query("SELECT pv.* from programas_vigencia AS pv JOIN (orgaos AS og) ON (og.nome = pv.orgao OR og.nome = pv.orgao_vinculado) WHERE og.codigo = '{$orgao}' AND pv.qualificacao LIKE '%Proposta Volunt%' AND (pv.estados LIKE '%todos%' OR pv.estados LIKE '%{$estado[0]->municipio_uf_sigla}%')");
        } else if ($this->session->userdata('sistema') == 'E') {
            $query = $this->db->query("SELECT pv.* from programas_vigencia AS pv JOIN (orgaos AS og) ON (og.nome = pv.orgao OR og.nome = pv.orgao_vinculado) WHERE og.codigo = '{$orgao}' AND pv.atende LIKE '%estadual%' AND pv.qualificacao LIKE '%Proposta Volun%' AND (pv.estados LIKE '%todos%' OR pv.estados LIKE '%{$estado[0]->municipio_uf_sigla}%')");
        } else if ($this->session->userdata('sistema') == 'P') {
            $query = $this->db->query("SELECT pv.* from programas_vigencia AS pv JOIN (orgaos AS og) ON (og.nome = pv.orgao OR og.nome = pv.orgao_vinculado) WHERE og.codigo = '{$orgao}' AND pv.qualificacao LIKE '%Proposta Volunt%'");
        }
    ...
    function envia_email($email, $email1, $texto) {
        ...
        $this->email->from('marcelomoura@physisbrasil.com.br', 'Captação Recursos SIHS');
        $this->email->to($email);

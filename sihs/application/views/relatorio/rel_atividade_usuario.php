<div id="content" class="innerAll bg-white">
    ...
    <h1 class="bg-white content-heading border-bottom">Relatório Por Usuário</h1>
    ...
    <?php if ($dados_rel != null): ?>
		...
		';
                    ...
                    $labelDataEnvio = "";
                    if ($dados->TITULO == "Propostas Cadastradas Pelo Sistema")
                        $labelDataEnvio = "Data Envio";

                    echo "<tr style='background-color:#DCDCDC;'><td></td><td style='color:red;'>Data Criação</td><td>" . implode("/", array_reverse(explode("-", $dados->data))) . "</td><td style='color:red;'>{$labelDataEnvio}</td><td colspan='3'>" . implode("/", array_reverse(explode("-", $dados->data_envio))) . "</td></tr>";
                    if ($dados->id_siconv != NULL && $dados->id_siconv != "") {
                        echo "<tr><td></td><td>Código Siconv</td><td colspan='6'>" . $dados->id_siconv . "</td></tr>";
                    } else {
                        echo "<tr><td></td><td>Código Siconv</td><td colspan='6'>" . "Não enviada" . "</td></tr>";
                    }
                    echo "<tr><td></td><td>Município</td><td colspan='6'>" . $proponente_siconv_model->get_municipio_by_cnpj($dados->proponente)->municipio . "</td></tr>";

...
//        $this->autentica_siconv->open_page_programas_livre($this->cookie_file_path);
        ...
        do {
            ...
            if ($usuario_propnente == null && $senha_proponente == null) {
                ...
                $test = $this->autentica_siconv->new_init_siconv_do_login($usuario_propnente, $senha_proponente, $this->login, $this->cookie_file_path, false);
            }
        } while ($test != null);
...
//                }
    ...
    public function import_from_siconv_to_sihs() {
        ini_set('max_execution_time', 0);
        ...
        if ($this->input->get_post('codigos_propostas', TRUE) != false) {
            ...
            try {
                ...
                $this->alert("Erro ao importar propostas do siconv para o SIHS");
            }
        }

        $data['title'] = "Physis - Importar do Siconv";
        $data['main'] = 'in/import_from_siconv_to_sihs';
    ...
    public function import_from_siconv($codigo_proposta = null, $banco_atende = null, $nome_proposta = null, $redirect = true) {
        ...
        //$nome_proposta = 'Testes Import Siconv banco proposta SIHS';
                ...
                if ($gestores_proponente != null) {
                    foreach ($gestores_proponente as $g) {
                        ...
                        if ($gestor_user->tipo_gestor == 10) {
                            ...
                            $urlCabecalho = "secretaria.physisbrasil.com.br/sihs/layout/assets/images/cab_info_generico.gif";
                            $urlRodape = "secretaria.physisbrasil.com.br/sihs/layout/assets/images/rod_info_generico.png";
                            ...
                            if ($user->id_nivel == 2) {
                                $this->envia_email($origem, "manoel.carvalho.neto@gmail.com", null, $assunto, $mensagem);
                            }
                        } else {
                            $origem = "physisbrasil@gmail.com";
                            $assunto = "Atualizações de Informações de Propostas";
                            ...
                            $urlCabecalho = "secretaria.physisbrasil.com.br/sihs/layout/assets/images/cab_sihs_generico.png";
                            $urlRodape = "secretaria.physisbrasil.com.br/sihs/layout/assets/images/rod_sihs_generico.png";
...
//        $this->usuariomodel->enviar_email_cron_eliumar('Atualizacao de proposta finalizada em ' . date('d/m/Y H:i:s') . ' - Proponente: ' . $proponente . ' ano: ' . $ano);
    ...
    public function update_status_propostas() {
        ...
        foreach ($proponentes as $propo) {
            ...
            try {
//                $this->usuariomodel->enviar_email_cron('Exceutando chamada python para atualizar proposta. ' . date('d/m/Y H:i:s') . ' - Proponente: ' . $propo->cnpj . ' ano: ' . '2016');
//                $this->usuariomodel->enviar_email_cron_eliumar('Exceutando chamada python para atualizar proposta. ' . date('d/m/Y H:i:s') . ' - Proponente: ' . $propo->cnpj . ' ano: ' . '2016');
                $commandLine = 'python /var/www/sihs/python_scripts/atualiza_propostas_proponente.py' . ' ' . $propo->cnpj . ' 2017';
...
//                $this->usuariomodel->enviar_email_cron('Exceutando chamada python para atualizar proposta. ' . date('d/m/Y H:i:s') . ' - Proponente: ' . $propo->cnpj . ' ano: ' . '2016');
//                $this->usuariomodel->enviar_email_cron_eliumar('Exceutando chamada python para atualizar proposta. ' . date('d/m/Y H:i:s') . ' - Proponente: ' . $propo->cnpj . ' ano: ' . '2016');
                $commandLine = 'python /var/www/sihs/python_scripts/atualiza_propostas_proponente.py' . ' ' . $propo->cnpj . ' 2016';
                $command = escapeshellcmd($commandLine);
                exec($command);
            } catch (Exception $e) {
//                $this->usuariomodel->enviar_email_cron('Falha ao executar chamada python para atualizar proposta. ' . date('d/m/Y H:i:s') . ' - Proponente: ' . $propo->cnpj . ' ano: ' . '2015');
            ...
            try {
//                $this->usuariomodel->enviar_email_cron('Exceutando chamada python para atualizar proposta. ' . date('d/m/Y H:i:s') . ' - Proponente: ' . $propo->cnpj . ' ano: ' . '2014');
//                $this->usuariomodel->enviar_email_cron_eliumar('Exceutando chamada python para atualizar proposta. ' . date('d/m/Y H:i:s') . ' - Proponente: ' . $propo->cnpj . ' ano: ' . '2014');
                $commandLine = 'python /var/www/sihs/python_scripts/atualiza_propostas_proponente.py' . ' ' . $propo->cnpj . ' 2014';
                $command = escapeshellcmd($commandLine);
                exec($command);
            } catch (Exception $e) {
//                $this->usuariomodel->enviar_email_cron('Falha ao executar chamada python para atualizar proposta. ' . date('d/m/Y H:i:s') . ' - Proponente: ' . $propo->cnpj . ' ano: ' . '2014');
            }
...
//                $this->usuariomodel->enviar_email_cron_eliumar('Exceutando chamada python para atualizar proposta. ' . date('d/m/Y H:i:s') . ' - Proponente: ' . $propo->cnpj . ' ano: ' . '2015');
                $commandLine = 'python /var/www/sihs/python_scripts/atualiza_propostas_proponente.py' . ' ' . $propo->cnpj . ' 2015';
                $command = escapeshellcmd($commandLine);
                exec($command);
            } catch (Exception $e) {
//                $this->usuariomodel->enviar_email_cron('Falha ao executar chamada python para atualizar proposta. ' . date('d/m/Y H:i:s') . ' - Proponente: ' . $propo->cnpj . ' ano: ' . '2015');
    ...
    public function check_aproved($string) {
        ...
        if (count($array) > 0) {
            foreach ($array as $palavra) {
                if ($palavra == "Aprovada" || $palavra == "Aprovados") {
                    return true;
                }
            }
        }
    ...
    public function update_empenho_prosposta_sihs($id_proposta, $id_siconv) {
        ini_set('max_execution_time', 0);
        $this->load->model('proposta_model');
        ...
        if ($banco_proposta['empenhado'] != null && $banco_proposta['empenhado'] == 'sim') {
            ...
            if ($empenhos != null && count($empenhos) > 0) {
                ...
                foreach ($empenhos as $empenho) {
                    //adicionar empenhos novos ou atualizar os existentes
                    $this->empenhos_sihs_model->add_or_update_empenho($empenho);
                }
            }
                            ...
                            if ($query_hash_desconto->num_rows == 0) {
                                ...
                                if ($resultado->codigo_ref_compra == null) {
                                    ...
                                    if ($query_hash_livre_topo->num_rows > 0) {
                                        ...
                                        $urlCabecalho = "secretaria.physisbrasil.com.br/sihs/layout/assets/images/cab_info_generico.gif";
                                        $urlRodape = "secretaria.physisbrasil.com.br/sihs/layout/assets/images/rod_info_generico.png";
                                        $urlBotao = "secretaria.physisbrasil.com.br/sihs/layout/assets/images/bt_info_comprar.png";
...
//            var_dump($lista_proposta_envia_email);
                foreach ($lista_proposta_envia_email as $cnpjKey => $codigos_envia_email) {
                    ...
                    if ($dados_gestor->tipo_gestor == 10) {
                        ...
                        $urlCabecalho = "secretaria.physisbrasil.com.br/sihs/layout/assets/images/cab_alt_prop_novo.gif";
                        $urlRodape = "secretaria.physisbrasil.com.br/sihs/layout/assets/images/rodape_emails_novo.png";
    ...
    public function enviar_notificacao_manualmente() {
        #Variaveis com os emails dos destinatários e as propostas que sofreram alteração de status ou pareceres.
        $lista_destinatários = array();
        $lista_propostas = array();
        $destinatario_primario = "";
        $texto_destinarios_cc = NULL;

        foreach ($lista_destinatários as $destinario) {
            if ($texto_destinarios_cc == NULL) {
                $texto_destinarios_cc = $destinario;
            } else {
                $texto_destinarios_cc .= ", " . $destinario;
            }
        }
        ...
        $this->envia_email($origem, $destinatario_primario, $texto_destinarios_cc, $assunto, $mensagem);
    }
...
//            $this->usuariomodel->enviar_email_cron('Serviço iniciado em ' . date('d/m/Y H:i:s') . ' - CNPJ: ' . $d->cnpj);
            ...
            $commandLine = 'python /var/www/sihs/python_scripts/request_proposta_usuario.py' . ' ' . $d->cnpj;
            $command = escapeshellcmd($commandLine);
            echo $command . "<br>";
            $output = exec($command);
            echo $output . "<br><br>";
            ...
            foreach ($gestores_info_convenios as $g_data) {
                ...
                if ($propostas_from_proponente_fake_gestor != null) {
                    foreach ($propostas_from_proponente_fake_gestor as $prop) {
                        //Consulta
                        $commandLine = 'python /var/www/sihs/python_scripts/request_proposta_from_id.py' . ' ' . $prop->id_proposta . ' ' . $g_data->id_usuario;
                        $command = escapeshellcmd($commandLine);
                        exec($command);
                        sleep(5);
                    }
...
//        $this->usuariomodel->enviar_email_cron('Serviço iniciado em ' . date('d/m/Y H:i:s'), "CNPJs Vendedores");
        ...
        foreach ($dados as $d) {
            $commandLine = 'python /var/www/sihs/python_scripts/request_proposta_usuario.py' . ' ' . $d->cnpj_vinculado;
            $command = escapeshellcmd($commandLine);
            echo $command . "<br>";
            $output = exec($command);
            echo $output . "<br><br>";
...
//        $this->usuariomodel->enviar_email_cron('Serviço finalizado em ' . date('d/m/Y H:i:s'), "CNPJs Vendedores");
    ...
    function envia_email($origem, $destino, $copia, $assunto, $mensagem) {
        ...
        $this->email->from($origem, "Physis Brasil -- SIHS");
...
//                $this->usuariomodel->enviar_email_cron('Serviço iniciado em ' . date('d/m/Y H:i:s') . "<br/>- Parlamentar " . trim($c) . " - {$parlamentar->nome_parlamentar}", "Propostas Parlamentares - Deputado");
                ...
                $commandLine = 'python /var/www/sihs/python_scripts/request_proposta_parlamentar.py' . ' ' . trim($c);
                $command = escapeshellcmd($commandLine);
                echo $command . "<br>";
                $output = exec($command);
                echo $output . "<br><br>";

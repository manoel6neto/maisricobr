...
class Dados_siconv extends BaseController {
    ...
    function busca_programas($offSet = 0) {
        ...
        $data['chave'] = array('Abastecimento de Água', 'Saneamento Básico', 'Resíduos Sólidos', 'Recursos Hídricos', 'Saúde', 'Turismo', 'Sustentabilidade',
            'Educação Ambiental', 'Comunidades Tradicionais', 'Assentamentos', 'Agricultura Familiar', 'Irrigação', 'Segurança Hídrica', 'Segurança de Barragem');
        ...
        $data ['title'] = "SIHS - Busca de Programas"; // titulo da pagina
    ...
    public function busca_emendas() {
        ...
        $data ['title'] = "SIHS - Busca de Emendas"; // titulo da pagina
...
//         var_dump($data['emendas']);
    ...
    public function rel_emendas_pdf() {
        ...
        $data ['title'] = "SIHS - Busca de Emendas";
        $data['programa_model'] = $this->programa_model;
    ...
    public function visualiza_propostas_pareceres() {
        $geral = $this->input->get_post('geral', TRUE);

        if ($geral != FALSE) {
            $this->session->set_userdata('pagAtual', 'visualiza_propostas_pareceres_geral');
        } else {
        ...
        if ($this->session->userdata('nivel') == 6 || $this->session->userdata('nivel') == 7 || $this->session->userdata('nivel') == 8)
            $data['num_cnpj'] = count($this->usuario_cnpj->get_all_by_subgestor($this->session->userdata('id_usuario')));
        else
        ...
        if ($geral == 1) {
            $data['dados_propostas'] = $this->banco_proposta_model->busca_programas_pareceres($this->session->userdata('filtros'), false, array(), 1);
        } else {
        ...
        $data['title'] = 'SIHS - Propostas e Pareceres';

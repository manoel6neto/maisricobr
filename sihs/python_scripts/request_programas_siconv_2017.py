...
url_consulta = str('http://localhost/esicar/index.php/get_programas/get_ids_programas?ano=2017')
...
url_consulta = str('http://localhost/esicar/index.php/get_programas/retorna_num_ids?ano=2017')
response = requests.get(url_consulta)
if response.status_code == 200:
...
if num_registros > 0:
	...
	while offset <= num_registros:
		try:
			url_consulta = str(str('\"') + 'http://localhost/esicar/index.php/get_programas/get_programas_siconv?ano=2017&offset=' + str(offset) + str('\"'))

...
PATH_SCRIPTS = "/home/physisroot/scripts/"
PATH_BACKUPS = PATH_SCRIPTS + "backupbd/"
SIZE_RANDOM = 10
FINAL_BACKUP_FILE_NAME = "backup_physis_esicar.sql"


def principal():
    status = Popen("/etc/init.d/apache2 stop", shell=True).wait()
    if status == 0:
        status = Popen("/etc/init.d/mysql restart", shell=True).wait()
        if status == 0:
            ...
            Popen("/etc/init.d/apache2 start", shell=True).wait()
	    Popen('sshpass -p "u66aH1iHo3" rsync -rav --progress -e "ssh -p 2222" /home/physisroot/scripts/backupbd/backup_physis_esicar.sql physi971@179.190.55.139:/home/physi971/backup_godaddy_esicar_database', shell=True).wait()


def executabackup(backup_file_name, final_file_name):
    os.chdir(PATH_BACKUPS)
    status = Popen("mysqldump -uroot -pPhysis_2013 physis_esicar -r %s" % backup_file_name, shell=True).wait()
    if status == 0:
        try:

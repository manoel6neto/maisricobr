...
<?php if(isset($_GET['id'])):?>
<label style="color: red;">Alteração de Senha</label>
<div class="form-group">
	<?php echo form_label('Senha Login Info Convênios', 'senha');?>
	<?php echo form_password(array('name'=>'senha', 'class'=>'form-control'), set_value('senha', '')); ?>
</div>
<?php endif;?>
...
<div class="form-group">
	<fieldset>
		...
		<?php echo form_radio('usuario_sistema', 'T', set_radio('usuario_sistema', 'T', (isset($usuario) && $usuario->usuario_sistema == 'T') ? true : false), 'class="usuario_sistema"');?>
		<?php echo form_label('Todos', 'tipo_gestor');?>
		<?php echo form_radio('usuario_sistema', 'M', set_radio('usuario_sistema', 'M', (isset($usuario) && $usuario->usuario_sistema == 'M') ? true : false), 'class="usuario_sistema"');?>
		<?php echo form_label('Info Convênios Municipio', 'usuario_sistema');?>
		<?php echo form_radio('usuario_sistema', 'P', set_radio('usuario_sistema', 'P', (isset($usuario) && $usuario->usuario_sistema == 'P') ? true : false), 'class="usuario_sistema"');?>
		<?php echo form_label('Info Convênios Parlamentar', 'usuario_sistema');?>
		<?php echo form_radio('usuario_sistema', 'E', set_radio('usuario_sistema', 'E', (isset($usuario) && $usuario->usuario_sistema == 'E') ? true : false), 'class="usuario_sistema"');?>
		<?php echo form_label('Info Convênios Estadual', 'usuario_sistema');?>
	</fieldset>
</div>
<?php endif;?>
...
<div id="dadosSiconv" hidden="true">
	<label hidden="true" style="color: #428bca;">DADOS SICONV</label>
	<div class="form-group" hidden="true">
...
foreach($usuariosGestor as $gestor){
	if($this->session->userdata('nivel') == 2 && $gestor->id_usuario == $this->session->userdata('id_usuario'))
		$usuarios[$gestor->id_usuario] = $gestor->nome . ($gestor->usuario_sistema == "M" ? " - Municipal" : " - Parlamentar");
	else
		$usuarios[$gestor->id_usuario] = $gestor->nome . ($gestor->usuario_sistema == "M" ? " - Municipal" : " - Parlamentar");
}
?>
...
<div id="dadosVendedor">
	<label>Restringir Acesso do Vendedor</label>
	<div class="form-group">
		<?php echo form_label('Estado', 'estado'); ?>
		<?php echo form_multiselect('estado_restrito[]', $proponente_siconv_model->getListaEstados(), $estados_block, "id='estado_restrito' class='form-control' style='display: none;'"); ?>
	&nbsp;
		<?php echo form_label('Esfera Administrativa', 'esfera'); ?>
		<?php echo form_multiselect('esfera_restrita[]', $proponente_siconv_model->getListaEsferas(), $esferas_block, "id='esfera_restrita' class='form-control' multiple='multiple' style='display: none;'"); ?>
	</div>
</div>
<?php endif;?>
...
<b><a href='https://www.convenios.gov.br/siconv/secure/entrar-login.jsp' onclick="window.open ('https://www.convenios.gov.br/siconv/secure/entrar-login.jsp','mywindow','menubar=1,resizable=1,width=800,height=600'); return false;" target='_blank' id="abre_siconv">Clique aqui para alterar sua senha no SICONV</a></b>
</p>
</div>

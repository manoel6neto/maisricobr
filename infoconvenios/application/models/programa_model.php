...
class programa_model extends CI_Model {
    ...
    function busca_programa($search = array(), $offSet = 0, $limit = 1000, $buscaEmendas = false) {
        ...
        if ($this->session->userdata('tipo_gestor') == 10) {
            $cnpjs_user = $this->usuariomodel->get_cnpjs_by_usuario($this->session->userdata('id_usuario'));
        }
        ...
        if ($this->session->userdata('nivel') != 1) {
            ...
            if (($this->session->userdata('usuario_sistema') == "T" && $this->session->userdata('nivel') == 4) || $this->session->userdata('usuario_sistema') == "M") {
                $i = 1;
                $array = "(";
                $lista_estados = "";
        ...
        if ($search == null) {
            if ($this->session->userdata('sistema') != "P") {
                $lista_filtro_esferas = array(
                    ...
                    'privada' => "Entidade Privada sem fins lucrativos"
                );
                ...
                if ($buscaEmendas) {
                    $this->db->where("(`atende` like '%Administração Pública Municipal%' OR `atende` like '%Consórcio Público%' OR `atende` like '%Administração Pública Estadual ou do Distrito Federal%' OR `atende` like '%Entidade Privada sem fins lucrativos%' OR `atende` like '%Empresa pública/Sociedade de economia mista%')");
                    $this->db->where("`qualificacao` like '%Proposta Voluntária%'");
                }
            }
            ...
            if (isset($search['cnpj']) || $this->session->userdata('tipo_gestor') == 10) {
                if ($this->session->userdata('tipo_gestor') != 10) {
    ...
    function programas_abertos($usuarioNovo = false, $idUsuario = null) {
        ...
        if (!empty($gestores)) {
            foreach ($gestores as $gestor) {
                ...
                if ($gestor->id_nivel != 5 && $gestor->id_nivel != 3) {
                ...
                $this->prepara_email($gestor->email, $lista_encarregados, $array, $usuarioNovo);
                ...
                if (!$usuarioNovo) {
                    ...
                    if (!empty($cnpjs)) {
                        ...
                        $this->prepara_email_emendas($gestor->email, $lista_encarregados, $array, $listaCNPJs, $usuarioNovo);
                    } else {
                ...
                if ($gestor->usuario_sistema == 'P') {
                    ...
                    $this->prepara_email_vencimento($gestor->email, $lista_encarregados, 5, $array); //vencimento 5 dias a frente
                    $this->prepara_email_vencimento($gestor->email, $lista_encarregados, 0, $array); //vencimento no dia do envio
                }
            }
        }

        if (!$usuarioNovo)
            $this->prepara_email("eliumar@gmail.com", "miguelaugusto5@yahoo.com.br, manoel.carvalho.neto@gmail.com, lgmafra@gmail.com, maxflavio12@gmail.com, allan@physisbrasil.com.br, psansao@physisbrasil.com.br, psansao@gmail.com");

        $this->marca_programas_antigos();
    }

    public function prepara_email_vencimento($email, $email_cc, $num_dias, $filtro_estados = "") {
    ...
    public function prepara_email_emendas($email, $email_cc, $filtro = "", $listaCNPJs = null, $usuarioNovo = false, $numeroParlamentar = null, $numDias = null) {
        if (!$usuarioNovo) {
    ...
    public function prepara_email($email, $email_cc, $filtro = "", $usuarioNovo = false) {
        if (!$usuarioNovo) {
    ...
    function envia_email($email, $email1, $texto, $assunto) {
        $this->load->model('usuariomodel');

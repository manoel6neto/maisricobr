...
class controle_usuarios extends BaseController {
    ...
    public function index() {
        ...
        $data['title'] = 'Info Convênios - Controle de Usuários';
    ...
    public function atualiza_encarregados($id_gestor_default = null) {
        ...
        $data['title'] = 'Info Convênios - Atualização de Gestores';
    ...
    public function novo_encarregado() {
        ...
        $data['title'] = 'Info Convênios - Novo Responsável';
    ...
    public function editar_encarregado() {
        ...
        $data['title'] = 'Info Convênios - Atualizar Responsável';
        $data['main'] = "controle_usuarios/novo_encarregado";
        $data['id_gestor'] = $id_gestor;
    ...
    public function novo_usuario() {
        ...
        $data['title'] = 'Info Convênios - Novo Usuário';
    ...
    public function envia_sms($nome_usuario) {
        ...
        $telefones = array('557388223469', '557388462781', '557388466588', '557391934769', '557391192425');
        ...
        for ($i = 0; $i <= 4; $i++) {
            ...
            $msg = "Caro administrador,\r\n O usuário {$nome_usuario} foi cadastrado no sistema Info Convênios."; // Mensagem
    ...
    public function atualiza_usuario() {
        ...
        if ($this->input->post(null, TRUE)) {
            $this->form_validation->set_rules('nome', 'Nome', 'required');
            $this->form_validation->set_rules('telefone', 'Telefone', '');
            $this->form_validation->set_rules('senha', 'Senha Login Info Convênios', '');
            $this->form_validation->set_rules('celular', 'Celular', '');
            $this->form_validation->set_rules('login_siconv', 'Login SICONV', '');
            $this->form_validation->set_rules('senha_siconv', 'Senha SICONV', '');
            $this->form_validation->set_rules('id_nivel', 'Nível', 'required');
        ...
        $data['title'] = 'Info Convênios - Atualização de Usuário';
        $data['main'] = "controle_usuarios/novo_usuario";
        $data['nivelUsuarios'] = $nivelUsuarios;
        $data['usuariosGestor'] = $usuariosGestor;
    ...
    public function vincular_cnpj() {
        ...
        $data['usuariomodel'] = $this->usuariomodel;
        $data['title'] = 'Info Convênios - Vincular CNPJ';
        $data['main'] = $pagina;
    ...
    public function vincular_codigo_parlamentar_vendedor() {
        ...
        $data['title'] = 'Info Convênios - Vincular Código Parlamentar';
        $data['main'] = $pagina;
    ...
    public function permissao_usuario() {
        ...
        $data['title'] = 'Info Convênios - Permissões Usuário';
        $data['main'] = "controle_usuarios/permissao_usuario";
    ...
    public function ativa_usuario_avulso($id = null, $ref = null) {
        ...
        if ($ref == null) {
            if ($this->input->get('cod_ref', TRUE) !== FALSE) {
                $ref = $this->input->get('cod_ref', TRUE);
            }
        }

        if ($id != null || $id != '') {
            $this->load->model('usuariomodel');
            $num_rows = $this->usuariomodel->muda_status_usuario($id, 'A');
            ...
            if ($num_rows > 0) {
                ...
                if ($dados_usuario->id_nivel == 2) {
                    if (true) {
                        ...
                        $this->pgu->set_permissao_plano($id, $ref);
                        ...
                        $this->usuariomodel->marca_usuario_nao_novo($id);

                        $this->programa_model->programas_abertos(true, $id);
                    }
                }

                $this->load->model('system_logs');
                $this->system_logs->add_log(USUARIO_ATIVADO . " - ID: " . $id . ", Nome: " . $this->usuariomodel->get_by_id($id)->nome);
    ...
    public function sumario() {
        ...
        $data['usuario'] = $this->pgu->get_user_by_id($this->input->get('id', TRUE));
        $array_cnpjs = $this->usuario_cnpj->get_cnpjs_gestor_object($this->input->get('id', TRUE));
        $cnpj_cidade = $array_cnpjs[0];
        $data['cnpj'] = $cnpj_cidade;
        $data['gestor'] = $this->gestor->get_by_usuario($this->input->get('id', TRUE));
        $data['proponente'] = $this->proponente_siconv_model->get_municipio_nome($cnpj_cidade->id_cidade);
        
        $data['title'] = "Info Convênios - Controle de Usuários _ Sumário";
        $data['main'] = 'controle_usuarios/sumario';
        $this->load->view('controle_usuarios/temp_controle_usuarios', $data);
    }
    ...
    public function confirma_pagamento() {
        ...
        if (!$this->usuariomodel->is_new($this->input->get('id', TRUE))) {
            $this->usuariomodel->amplia_ou_renova_validade($this->input->get('id', TRUE), $this->input->get('cod_ref', TRUE));
            $this->pgu->set_permissao_plano($this->input->get('id', TRUE), $this->input->get('cod_ref', TRUE));
            $this->pgu->ativa_compra($this->input->get('id', TRUE), $this->input->get('cod_ref', TRUE));
        } else {
            $this->pgu->set_permissao_plano($this->input->get('id', TRUE), $this->input->get('cod_ref', TRUE));
            $this->ativa_usuario_avulso($this->input->get('id', TRUE), $this->input->get('cod_ref', TRUE));
            $this->pgu->ativa_compra($this->input->get('id', TRUE), $this->input->get('cod_ref', TRUE));
        }
        
        $this->pgu->ativar_plano_desativar_restantes($this->input->get('id', TRUE), $this->input->get('cod_ref', TRUE));
...
//         }
    ...
    public function cadastro_visitas() {
        ...
        $data['title'] = 'Info Convênios - Cadastro de Visitas';

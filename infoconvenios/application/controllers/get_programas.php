...
class get_programas extends CI_Controller {
    ...
    public function get_programas_siconv($offset = 0) {
        ini_set('max_execution_time', 0);
        ...
        if ($this->input->get('offset', TRUE) == 0)
            $this->usuariomodel->enviar_email_cron('Programas - Serviço iniciado em ' . date('d/m/Y H:i:s'), "Programas - " . $this->input->get('ano', TRUE) . " - " . date('d/m/Y H:i:s'));

        $listaIDS = $this->buscar_ids($this->input->get('ano', TRUE), $this->input->get('offset', TRUE));

        if (empty($listaIDS)) {
            $this->usuariomodel->enviar_email_cron('Programas - Serviço finalizado em ' . date('d/m/Y H:i:s'), "Programas - " . $this->input->get('ano', TRUE) . " - " . date('d/m/Y H:i:s'));

...
class compra extends CI_Controller {
    ...
    public function index() {
        ...
        if (base64_decode($token_url) === $this->token) {
            ...
            $id_usuario = $this->input->get('desconto', TRUE);
            $hash_object = $this->hash_model->get_usuario($id_usuario);
            if ($hash_object != null) {
                $user = $this->usuariomodel->get_by_id($hash_object->id_usuario);
                if ($user->data_validade_desconto != null) {
                    if (time() <= strtotime($user->data_validade_desconto)) {
                        $data['desconto'] = $id_usuario;
                    } else {
                        $this->alert("Prazo de desconto encerrado !!");
                        $this->encaminha("http://www.info-convenios.com/");
                    }
                } else {
                    $this->alert("Falha ao processar desconto !! Favor consultar administrador do sistema !!");
                    $this->encaminha("http://www.info-convenios.com/");
                }
            } 
            ...
            $data['title'] = "Info Convênios - Compra";
    ...
    public function resumo_pedido() {
        if ($this->input->post()) {
            ...
            if ($this->input->post('desconto', TRUE)) {
                $data['desconto'] = $this->input->post('desconto', TRUE);
            }
        }
    ...
    public function cadastro_usuario() {
        ...
        if ($this->input->get_post('desconto', TRUE) != false) {
            $data['desconto'] = $this->input->get_post('desconto', TRUE);
        }
        ...
        if ($this->input->post('login', TRUE)) {
            ...
            if ($this->form_validation->run()) {
                ...
                if (!is_null($retorno) || $this->input->get_post('gratuito', TRUE) != false) {
                    ...
                    $hash_desconto = null;
                    if ($this->input->get_post('desconto', TRUE) != false) {
                        $hash_desconto = $this->input->get_post('desconto', TRUE);

                        $hash_object = $this->hash_model->get_by_hash($hash_desconto);
                        if ($hash_object != null) {
                            if ($hash_object->id_usuario == null) {
                                $optionsUsuario['tem_desconto'] = 1;
                            } else {
                                $this->alert('Hash associada a outro usuario. Entre em contato com o suporte.');
                                $this->encaminha("http://www.info-convenios.com/");
                                return null;
                            }
                        } else {
                            $this->alert('Hash invalida. Entre em contato com o suporte.');
                            $this->encaminha("http://www.info-convenios.com/");
                            return null;
                        }
                    }
                    unset($optionsUsuario['desconto']);

                    $ultimo_usuario = $this->usuariomodel->insere_usuario($optionsUsuario);

                    if ($this->input->get_post('desconto', TRUE) != false) {
                        //Associa o usuario a hash
                        $this->hash_model->mark_hash_for_user($ultimo_usuario, $hash_desconto);
                    }
...
//                        //Ativando o plano e desativando qualquer outro plano restante - se existir na hora do cadastro algum restante n usou renovacao portante n acrescente o tempo no vencimento
//                        if ($this->input->get_post('gratuito', TRUE) == false) {
//                            $this->pgu->ativar_plano_desativar_restantes($ultimo_usuario, null);
//                        } else {
//                            $this->pgu->ativar_plano_desativar_restantes($ultimo_usuario, $retorno['refCod']);
//                        }

                        if ($this->input->get_post('gratuito', TRUE) != false) {
                            $this->encaminha("http://www.info-convenios.com/#!cadastro-confirmado/pn2a5");
                        } else {
        ...
        $data['title'] = "Info Convênios - Cadastro de Usuário";
    ...
    public function reativa_plano() {
        ...
        $desconto = null;
        if ($this->input->post('desconto', TRUE) !== false) {
        ...
        if ($this->input->post()) {
            ...
            if ($desconto != null) {
                if ($desconto != $dados_usuario->id_usuario) {
                    $this->alert('Usuario diferento do disponivel para ativacao com desconto. Favor entre em contato com o suporte ou logue com sua conta existente.');
                    $this->encaminha("http://www.info-convenios.com/");
                    return null;
                }
            }
            ...
            if (!is_null($dados_usuario)) {
                ...
                if (!is_null($retorno)) {
                    if ($dados_usuario->id_usuario != null) {
                        ...
                        if ($desconto == null) {
                            $this->usuariomodel->retira_desconto($dados_usuario->id_usuario);
                        }

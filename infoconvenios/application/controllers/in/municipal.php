...
class Municipal extends CI_Controller {
        ...
        if ($this->input->get_post('codigo', TRUE) !== false) {
            ...
            if ($this->usuario_model->verifica_codigo($codigo) == true) {
                if ($this->input->get_post('acao', TRUE) == 'mudar') {
                    ...
                    if ($this->usuariomodel->altera_senha($nova_senha, $codigo, $id_codigo, $email_codigo) > 0) {
                        $this->alert('A senha foi modificada com sucesso!');
                        $this->encaminha('../municipal');
                    }
                }
            } else {
    ...
    function recupera() {
        ...
        if ($this->input->get_post('email', TRUE) !== false) {
            ...
            if ($this->usuariomodel->recupera_email($email, $nivel, 'municipal') != null) {
    ...
    function index() {
        $data['title'] = "Info Convênios - Login";
        // VALIDATION RULES
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['main'] = 'login/municipal';
            $this->load->view('in/template_login', $data);
        } else {
            $query = $this->usuariomodel->validar('M');

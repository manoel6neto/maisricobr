...
class Dados_siconv extends BaseController {
    ...
    function busca_programas($offSet = 0) {
        ...
        $result = $this->programa_model->busca_programa($data, $offSet);
        ...
        if ($this->session->userdata('nivel') == 2 || $this->session->userdata('nivel') == 3 || $this->session->userdata('nivel') == 4)
        ...
        $data ['title'] = "Info Convênios - Busca de Programas"; // titulo da pagina
    ...
    public function busca_emendas() {
        ...
        $data ['title'] = "Info Convênios - Busca de Emendas"; // titulo da pagina
...
//         var_dump($data['proponentes_emendas']);
//         var_dump($data['emendas']);
    ...
    public function busca_emendas_municipio($offSet = 0) {
        ...
        if ($this->session->userdata('tipo_gestor') == 10) {
            //emendas
            $cnpjs = $this->usuariomodel->get_cnpjs_by_usuario($this->session->userdata('id_usuario'));
            $array_cnpjs = array();
            foreach ($cnpjs as $cn) {
                array_push($array_cnpjs, $cn->cnpj);
            }

            $array_cnpj_formatado = array();
            foreach ($array_cnpjs as $cn_nf) {
                array_push($array_cnpj_formatado, substr($cn_nf, 0, 2) . '.' . substr($cn_nf, 2, 3) . '.' . substr($cn_nf, 5, 3) . '/' . substr($cn_nf, 8, 4) . '-' . substr($cn_nf, 12));
            }

            $this->db->join('programas_vigencia pv', 'pv.codigo = sb.codigo_programa');
            $this->db->where('emenda <> ""');
            $this->db->where_in('cnpj', $array_cnpj_formatado);
            $query_emendas_parl = $this->db->get('siconv_beneficiario sb');

            $data['num_emendas_parl'] = $query_emendas_parl->num_rows;

            $this->db->join('programas_vigencia pv', 'pv.codigo = sb.codigo_programa');
            $this->db->where('emenda = ""');
            $this->db->where_in('cnpj', $array_cnpj_formatado);
            $query_emendas_espc = $this->db->get('siconv_beneficiario sb');

            $data['num_emendas_especifica'] = $query_emendas_espc->num_rows;
        }
    ...
    function gerapdf_lista() {
        ...
        $entidade = $this->usuario_cnpj->get_all_by_usuario($data['cnpj'], true);
        $entidade = $entidade[0];
        $entidade = $this->cnpj_siconv->get_by_id($entidade->id_cnpj);
        ...
        $header = array(
            ...
            'R' => array(
                'content' => trim($entidade->cnpj_instituicao),
                'font-size' => 8
            ),
            'line' => 1
        );
        ...
        foreach ($resultados as $query) {
            ...
            if ($orgao == "" || $orgao != $query->orgao) {
                ...
                $tabela .= "<br /><br /><table style=\"font-size: 11px; page-break-inside: avoid;\">";
                //$tabela .= "<br /><br /><table style=\"font-size: 11px;\">";
                $mpdf->AddPage();
                $total = 0;
            } else {
                $tabela = "<br /><br /><table style=\"font-size: 11px; page-break-inside: avoid;\">";
            ...
            $tabela .= "<tr><td colspan=\"3\" style=\"color:#428bca;\">" . trim($query->orgao) . "</td></tr>";
            $tabela .= "<tr><td colspan='3'></td></tr>";
            $tabela .= "<tr><td colspan='3'></td></tr>";
            ...
            $tabela .= "<tr><td colspan=\"3\" style=\"text-align: justify;\">" . trim($query->qualificacao) . "</td></tr>";
            $tabela .= "<tr><td colspan='3'></td></tr>";
            $tabela .= "<tr><td colspan='3'></td></tr>";
            ...
            $tabela .= "<tr><td colspan=\"3\" style=\"text-align: justify;\">" . trim($query->descricao) . "</td></tr>";
            $tabela .= "<tr><td colspan='3'></td></tr>";
            $tabela .= "<tr><td colspan='3'></td></tr>";

            $tabela .= "<tr><td colspan='3'><b>OBJETO: </b></td></tr></table>";

            $tabela .= "<table style=\"font-size: 11px; page-break-inside: avoid;\">";
            $tabela .= "<tr><td style=\"text-align: justify;\">" . trim(str_replace("&nbsp;", "", $query->objeto)) . "</td></tr>";
            $tabela .= "</table>";
            ...
            if (isset($query->regra_contrapartida)) {
                ...
                $tabela .= "<tr><td align=\"center\" colspan=\"3\" style=\"text-align: center;\">" . str_replace("td", "td style=\"text-align:center\"", str_replace("tr class=\"even\"", "tr class=\"even\" style=\"text-align:center\"", str_replace("tr class=\"odd\"", "tr class=\"odd\" style=\"text-align:center\"", trim($query->regra_contrapartida)))) . "</td></tr>";
            }
            $tabela .= "<tr><td colspan='3'></td></tr>";
            $tabela .= "<tr><td colspan='3'></td></tr>";

            $tabela .= "</table><br> <font style=\"font-size: 11px; text-align: justify;\">";
            $tabela .= "<b>Obs.: </b> " . trim($query->observacao) . "</font><br>";
    ...
    public function envia_programas_email() {
        ...
        foreach ($resultados as $query) {
            ...
            $tabela .= "<tr><td colspan='3'><b>ÓRGÃO SUPERIOR: </b></td></tr>";
            $tabela .= "<tr><td colspan=\"3\" style=\"color:#428bca;\">" . $query->orgao . "</td></tr>";
            $tabela .= "<tr><td colspan='3'></td></tr>";
            $tabela .= "<tr><td colspan='3'></td></tr>";
            ...
            $tabela .= "<table style=\"font-size: 11px; page-break-inside: avoid;\">";
            $tabela .= "<tr><td style=\"text-align: justify;\">" . $query->objeto . "</td></tr>";
            $tabela .= "</table>";
        ...
        $from_mail = 'no-reply@info-convenios.com.br';
    ...
    public function visualiza_propostas_pareceres() {
        ...
        $data['title'] = 'Info Convênios - Propostas e Pareceres';
    ...
    public function gerapdf_lista_propostas() {
        ...
        $dados_propostas = $this->banco_proposta_model->get_dados_by_ids($this->input->post('ids', TRUE));
        if ($this->session->userdata('nivel') == 6 || $this->session->userdata('nivel') == 7 || $this->session->userdata('nivel') == 8)
    ...
    function carrega_programas() {
        $data ['title'] = "Info Convênios - Gestão de Usuários e Propostas";
    ...
    function pdf1_programas() {
        ...
        // $tabela = "Info Convênios CONSULTORIA E ASSESSORIA SOLIDÁRIA";
        $tabela = "";

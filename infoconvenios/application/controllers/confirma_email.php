...
class confirma_email extends CI_Controller {
        ...
        if ($this->input->post()) {
            ...
            if ($this->input->get('token', TRUE) !== false) {
                ...
                if ($dados_cadastro == null || count($dados_cadastro) <= 0)
                    $this->alert("CPF ou token inválidos. Entre em contato com o contato@info-convenios.com");
                else {
                    ...
                    if ($usuario->id_nivel == 2) {
                        ...
                        if ($query_g->num_rows > 0) {
                            ...
                            if ($query_g->row(0)->tipo_gestor == 10) {
                                ...
                                $this->db->select('codigo_ref_compra');
                                $this->db->where('id_usuario', $usuario->id_usuario);
                                $result_consult = $this->db->get('pagseguro_usuario');

                                if ($result_consult->num_rows > 0) {
                                    $test = $result_consult->result();
                                    if ($test[0]->codigo_ref_compra == null || $test[0]->codigo_ref_compra == '') {
                                        //Chama funcao de ativar o usuario
                                        $this->ativa_usuario_avulso_gratuito_direto($usuario->id_usuario);
                                        $this->encaminha("http://www.info-convenios.com/#!dados-de-acesso/bze6s");
                                    } else {
                                        $this->usuariomodel->marca_usuario_nao_novo($usuario->id_usuario);
                                        $this->envia_sms_usuario_pagseguro($usuario->nome, $test[0]->codigo_ref_compra);
                                        $this->encaminha("http://www.info-convenios.com/#!dados-de-acesso/bze6s");
                                    }
                                }
                            } else {
                ...
                $this->alert("Token de identificação inválido. Entre em contato com o contato@info-convenios.com");
            }
        }

        $data['title'] = 'Info Convênios - Confirmação de Cadastro';
    ...
    public function envia_sms($nome_usuario) {
        //  		Max				Eliumar			Daiana			Allan      Thomas
    ...
    public function envia_sms_usuario_pagseguro($nome_usuario, $codigo_pagseguro) {
        //  		Max				Eliumar			Daiana			Allan     Thomas
        $telefones = array('5573988223469', '5573988462781', '5573988466588', '5573991934769', '5573991192425');
        ...
        for ($i = 0; $i <= 4; $i++) {
            ...
            $mobile = URLEncode($telefones[$i]); //* Numero do telefone  FORMATO: PAÍS+DDD(DOIS DÍGITOS)+NÚMERO
            $sendproj = URLEncode("N"); //* S = Envia o Remetente do SMS antes da mensagem , N = Não envia o Remetente do SMS
            $msg = "Caro administrador,\r\n O usuário {$nome_usuario} - {$codigo_pagseguro} foi cadastrado no sistema Info Convênios."; // Mensagem
            $msg = mb_convert_encoding($msg, "UTF-8"); // Converte a mensagem para não ocorrer erros com caracteres semi-gráficos
            $msg = URLEncode($msg);
            $response = fopen("http://www.mpgateway.com/v_2_00/smspush/enviasms.aspx?CREDENCIAL=" . $credencial . "&PRINCIPAL_USER=" . $principal . "&AUX_USER=" . $auxuser . "&MOBILE=" . $mobile . "&SEND_PROJECT=" . $sendproj . "&MESSAGE=" . $msg, "r");
            $status_code = fgets($response, 4);

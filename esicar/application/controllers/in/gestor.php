...
class Gestor extends BaseController {
    ...
    function index($offSet = 0) {
        ...
        if ($this->session->userdata('gp') == true || $this->input->get('gp', TRUE) != false) {
            $data['title'] = "G&P - Captação de Recursos";
        } else {
            $data['title'] = "Physis - e-Sicar Home";
        }
        $data['main'] = 'in/home_gestor2';
        $data['login'] = $this->login;

        /* Obtem CNPJ dos proponentes vinculados */
        $lista_cnpjs = array();
        ...
        if ($this->session->userdata('sistema') !== 'P') {
            $data['vencimento_dia'] = $this->programa_model->get_programas_vencimento(0, "", $lista_cnpjs);
            $data['vencimento_cinco_dias'] = $this->programa_model->get_programas_vencimento(5, "", $lista_cnpjs);
            $data['vencimento_dez_dias'] = $this->programa_model->get_programas_vencimento(10, "", $lista_cnpjs);
            if ($this->session->userdata('nivel') != 1) {
                $data['propostas_por_ano'] = $this->proposta_model->get_propostas_por_ano($this->session->userdata('id_usuario'));
            } else {
                $data['propostas_por_ano'] = array();
            }
        } else {
            $data['vencimento_dia'] = array();
            $data['vencimento_cinco_dias'] = array();
            $data['vencimento_dez_dias'] = array();
            $data['propostas_por_ano'] = array();
        }
    ...
    function criar_sala_conferencia() {
        $nome = urlencode($this->usuario_logado->nome . " - Physis");
    ...
    function incluir_justificativa() {
        ...
        if ($this->input->get_post('id', TRUE) !== false && $this->input->get_post('edit', TRUE) !== false && $this->input->get_post('justificativa', TRUE) !== false) {
            ...
            $data['id'] = $id;
            if ($this->session->userdata('gp') == true || $this->input->get('gp', TRUE) != false) {
                $data['title'] = "G&P - Incluir Justificativa";
            } else {
...
//$this->obter_pagina_post($pagina, $fields, $fields_string);
    ...
    function atribui_permissoes() {
        ...
        $data['login'] = $this->login;
        $data['title'] = "Physis - Atribui Permissões";
...
//         	}
            ...
            if ($this->nivel_usuario == "Representante Comercial") {
            ...
            $data['login'] = $this->login;
            $data['title'] = "Physis - Selecionar Programas";
    ...
    public function informa_valores_programa() {
        ...
        if (($this->input->get_post('cnpjProponente', TRUE) != false && $this->input->get_post('orgao', TRUE) !== false) || ($this->input->get('id', TRUE) != false && $this->input->get('edit', TRUE) != false)) {
            ...
            $force_offline = true;
            if ($this->session->userdata('nivel') == 1 || $this->session->userdata('nivel') == 4 || $this->session->userdata('nivel') == 2 || $this->session->userdata('nivel') == 15) {
                $force_offline = false;
            }
            ...
            if ($this->input->get('new_program', TRUE) == '1') {
                $this->load->model("programa_banco_proposta_model");
                $data['new_program_valores'] = $this->programa_model->get_by_siconv_id($this->input->post('idRowSelectionAsArray')[0]);
            }
            ...
            if ($this->input->post('id_programa_proposta', TRUE) != FALSE) {
                ...
                for ($i = 0; $i < count($valores['id_programa']); $i++) {
                    if (in_array($valores['id_programa'][$i], $valores['idRowSelectionAsArray'])) {
                        ...
                        $repasse_especifico = NULL;
                        $repasse_voluntario = NULL;
                        ...
                        $options_programa[$i] = array(
                            ...
                            'codigo_programa' => $valores['codigo_programa'][$i],
                            'programa' => "https://www.convenios.gov.br/siconv/programa/DetalharPrograma/DetalharPrograma.do?id=" . $valores['id_programa'][$i],
                            'percentual' => $valores['percentual'][$i] == "" ? "0.00" : str_replace(",", ".", substr($valores['percentual'][$i], 0, 4)),
                            'valor_global' => $valor_global,
                            'total_contrapartida' => $total_contrapartida,
                            'contrapartida_financeira' => $contrapartida_financeira == "" ? "0.00" : $contrapartida_financeira,
                            'contrapartida_bens' => $contrapartida_bens != '' ? $contrapartida_bens : NULL,
                            'repasse' => $repasse,
                            'repasse_voluntario' => $repasse_voluntario != '' ? $repasse_voluntario : NULL,
                            'repasse_especifico' => $repasse_especifico != '' ? $repasse_especifico : NULL,
                            'id_programa' => $valores['id_programa'][$i],
                            'objeto' => $valores['objeto'][$i],
                            'qualificacao' => $valores['qualificacao'][$i],
                            'id_programa_proposta' => $valores['id_programa_proposta'][$i],
                ...
                $options_programa[0]['cnpj_proponente'] = $valores['cnpjProponente'];
                $options_programa[0]['orgao'] = $valores['orgao'];
                $inserido = $this->trabalho_model->updateValoresProjeto($id, $options_programa[0]['valor_global'], $options_programa[0]['percentual'], $options_programa[0], isset($valores['opcao_crono_fisico']) ? $valores['opcao_crono_fisico'] : 2);
                    ...
                    for ($i = 0; $i < count($valores['id_programa']); $i++) {
                        if (in_array($valores['id_programa'][$i], $valores['idRowSelectionAsArray'])) {
                            ...
                            $options_programa[$i] = array(
                                ...
                                'codigo_programa' => $valores['codigo_programa'][$i],
                                'programa' => "https://www.convenios.gov.br/siconv/programa/DetalharPrograma/DetalharPrograma.do?id=" . $valores['id_programa'][$i],
                                'percentual' => str_replace(",", ".", substr($valores['percentual'][$i], 0, 4)),
                                'valor_global' => $valor_global,
                                'total_contrapartida' => $total_contrapartida,
                                'contrapartida_financeira' => $contrapartida_financeira,
                                'contrapartida_bens' => $contrapartida_bens != '' ? $contrapartida_bens : NULL,
...
//                 }
        ...
        $data['bancos'] = $this->proposta_model->obter_bancos();
        $data['title'] = 'Physis - Dados do Projeto';
        $data['login'] = $this->login;
        $data['areas'] = $this->trabalho_model->obter_areas();
        $data['main'] = $pagina_proj_padrao != "" ? $pagina_proj_padrao : 'gestor/incluir_proposta';
        $this->load->vars($data);
    ...
    function subtitui_tags_justificativa($codigo_cidade_endereco, $texto) {
        ...
        if ($cidade_tag != null) {
            ...
            $texto = str_replace('[area]', number_format(str_replace(',', '.', $cidade_tag->area), 2, ",", "."), $texto);
            $texto = str_replace('[densidade]', number_format(str_replace(',', '.', $cidade_tag->densidade), 2, ",", "."), $texto);
...
//            $this->autentica_siconv->new_init_siconv_do_login($this->login_usuario, $this->senha, $this->login_siconv, $this->cookie_file_path);
            ...
            foreach ($programas as $programa) {
                //página inicial para a inserção dos primeiros dados******************************************************
                $txt1 = $this->getTextBetweenTags($botoes[$i], "<a href=\"javascript:document.location='", "';\" class=\"buttonLink\">Selecionar");
            ...
            $this->alert("Enviado com sucesso para o SICONV ! Confirme o dialogo e feche a janela para continuar !");
            sleep(1);
            die();
    ...
    function visualiza_propostas() {
        ...
        if ($this->session->userdata('nivel') == 4 || $this->session->userdata('nivel') == 15) {
            ...
            $data['main'] = 'gestor/visualiza_propostas_adm';
        } else {
            $data['cnpjs'] = $this->usuariomodel->get_lista_cidades_by_cnpj($usuario);
        }

        /*
         *  Propostas com cidade = null estão sendo retornadas para `$data['propostas_cadastradas']` e são inúteis aqui.
         *  TODO: Verificar função `get_all_ativo_cadastradas`. Não mexi porque não sei onde mais essa função é usada 
         *  e se esse é o comportamento esperado.
         */
        $data['propostas_cadastradas'] = $this->proposta_model->get_all_ativo_cadastradas($usuario, $filtro);

        //Remove as propostas com cidade (que são os proponentes) null
        $propostas_cadastradas_filtradas = array();
        foreach ($data['propostas_cadastradas'] as $proposta) {
            foreach ($data['cnpjs'] as $cidade => $cnpj) {
                if (in_array($proposta->proponente, $cnpj)) {
                    $this->db->select('nome');
                    $this->db->where("REPLACE(REPLACE(REPLACE(cnpj, '/', ''), '-', ''), '.', '') = {$proposta->proponente}");
                    $query = $this->db->get('proponente_siconv');
                    if ($query->num_rows > 0) {
                        $propostas_cadastradas_filtradas[$query->row(0)->nome][] = $proposta;
                    } else {
                        $propostas_cadastradas_filtradas[$proposta->cidade][] = $proposta;
                    }
                }
            }
        }

        $data['propostas_cadastradas'] = $propostas_cadastradas_filtradas;

        /**
         * Mesmo tratamento anteior para propostas enviadas
         */
        $data['propostas_enviadas'] = $this->proposta_model->get_all_ativo_enviadas($usuario, $filtro);
        $propostas_enviadas_filtradas = array();
        foreach ($data['propostas_enviadas'] as $proposta) {
            foreach ($data['cnpjs'] as $cidade => $cnpj) {
                if (in_array($proposta->proponente, $cnpj)) {
                    $this->db->select('nome');
                    $this->db->where("REPLACE(REPLACE(REPLACE(cnpj, '/', ''), '-', ''), '.', '') = {$proposta->proponente}");
                    $query = $this->db->get('proponente_siconv');
                    if ($query->num_rows > 0) {
                        $propostas_enviadas_filtradas[$query->row(0)->nome][] = $proposta;
                    } else {
                        $propostas_enviadas_filtradas[$proposta->cidade][] = $proposta;
                    }
                }
            }
        }

        $data['propostas_enviadas'] = $propostas_enviadas_filtradas;
    ...
    function visualiza_banco_propostas() {
        ...
        if ($this->session->userdata('gp') == true || $this->input->get('gp', TRUE) != false) {
            $data['title'] = 'G&P - Banco de propostas';
        } else {
            $data['title'] = 'Physis - Banco de propostas';
        }

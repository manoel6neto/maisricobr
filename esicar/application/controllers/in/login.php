...
class Login extends CI_Controller {
    ...
    function index() {
        ...
        if ($this->form_validation->run() != FALSE || $this->input->get('gp', TRUE) != FALSE) {
            if ($this->input->get('gp', TRUE) == 1) {
                $this->session->set_userdata('gp', TRUE);
                $login_gp = base64_decode($this->input->get('login', TRUE));
                $senha_gp = base64_decode($this->input->get('senha', TRUE));
                $query = $this->usuariomodel->validar($login_gp, $senha_gp);
            } else {
                ...
                if (is_array($query)) {
                    ...
                    if ($query->usuario_sistema != 'T') {
                        if ($query->status === "A") {
                            ...
                            if ($query->id_nivel === "4" || $query->id_nivel === "1" || $ehParlamentar || $query->id_nivel === "15") {
                                $options = array(
                                    'login_siconv' => '',
                                    'senha_siconv' => '',
                            ...
                            // Verifica se é Execução de obras
                            if (($query->id_nivel == 12) && $this->usuariomodel->verifica_possui_usuario_siconv() || ($query->id_nivel == 13) && $this->usuariomodel->verifica_possui_usuario_siconv() || ($query->id_nivel == 14) && $this->usuariomodel->verifica_possui_usuario_siconv()) {
    ...
    function sair() {
        ...
        $redirect_url = 'in/login';
        if ($this->session->userdata('gp') == 1) {
            $redirect_url = 'in/login?gp=1';
        }

...
class proposta_model extends CI_Model {
    ...
    //THOMAS
    public function testa_situacao_enviada($situacao) {
        if (strpos(strtolower($situacao), "cadastrados") !== false || strpos(strtolower($situacao), "cancelados") !== false) {
            return false;
        } else {
            return true;
        }
    ...
    public function testa_situacao_aprovada($situacao) {
        if (strpos($situacao, "Proposta/Plano de Trabalho Cadastrados") !== false || strpos($situacao, "Proposta/Plano de Trabalho em Complementação") !== false || strpos($situacao, "Proposta/Plano de Trabalho Rejeitados") !== false || strpos($situacao, "Proposta/Plano de Trabalho Rejeitados por Impedimento técnico") !== false || strpos($situacao, "Proposta/Plano de Trabalho em rascunho") !== false || strpos($situacao, "Proposta/Plano de Trabalho enviado para Análise") !== false || strpos($situacao, "Proposta/Plano de Trabalho em Análise") !== false || strpos($situacao, "Proposta/Plano de Trabalho complementado enviada para Análise") !== false || strpos($situacao, "Proposta/Plano de Trabalho Cancelados") !== false || strpos($situacao, "Cancelado") !== false || strpos($situacao, "Proposta/Plano de Trabalho complementado em Análise") !== false || strpos($situacao, "Proposta/Plano de Trabalho em rascunho") !== false || strpos($situacao, "Enviada para Análise Preliminar") !== false) {
            return false;
        } else {
            return true;
        }
    ...
    //THOMAS: Carrega propostas para os proponentes por ano
    public function get_propostas_por_ano($id_usuario) {
        $this->load->model('usuariomodel');
        $this->load->model('banco_proposta_model');

        $array_propostas_por_ano = array('2018' => array(), '2014' => array(), '2015' => array(), '2016' => array(), '2017' => array(),
            '2018envi' => array(), '2014envi' => array(), '2015envi' => array(), '2016envi' => array(), '2017envi' => array(),
            '2018apro' => array(), '2014apro' => array(), '2015apro' => array(), '2016apro' => array(), '2017apro' => array());

        $cnpjs = $this->usuariomodel->get_cnpjs_by_usuario($id_usuario);
        $cnpjs_list = array();
        if (count($cnpjs) > 0) {
            foreach ($cnpjs as $cnpj) {
                array_push($cnpjs_list, $cnpj->cnpj);
            }
        }

        $anos = array(2014, 2015, 2016, 2017, 2018);
        $propostas = $this->banco_proposta_model->get_propostas_by_proponentes_anos($cnpjs_list, $anos);

        if (count($propostas) > 0) {
            foreach ($propostas as $prop) {
                switch ($prop->ano) {
                    case '2018':
                        array_push($array_propostas_por_ano['2018'], $prop->codigo_siconv);
                        if ($this->testa_situacao_enviada($prop->situacao)) {
                            array_push($array_propostas_por_ano['2018envi'], $prop->codigo_siconv);
                            if ($this->testa_situacao_aprovada($prop->situacao)) {
                                array_push($array_propostas_por_ano['2018apro'], $prop->codigo_siconv);
                            }
                        }
                        break;
                    case '2014':
                        array_push($array_propostas_por_ano['2014'], $prop->codigo_siconv);
                        if ($this->testa_situacao_enviada($prop->situacao)) {
                            array_push($array_propostas_por_ano['2014envi'], $prop->codigo_siconv);
                            if ($this->testa_situacao_aprovada($prop->situacao)) {
                                array_push($array_propostas_por_ano['2014apro'], $prop->codigo_siconv);
                            }
                        }
                        break;
                    case '2015':
                        array_push($array_propostas_por_ano['2015'], $prop->codigo_siconv);
                        if ($this->testa_situacao_enviada($prop->situacao)) {
                            array_push($array_propostas_por_ano['2015envi'], $prop->codigo_siconv);
                            if ($this->testa_situacao_aprovada($prop->situacao)) {
                                array_push($array_propostas_por_ano['2015apro'], $prop->codigo_siconv);
                            }
                        }
                        break;
                    case '2016':
                        array_push($array_propostas_por_ano['2016'], $prop->codigo_siconv);
                        if ($this->testa_situacao_enviada($prop->situacao)) {
                            array_push($array_propostas_por_ano['2016envi'], $prop->codigo_siconv);
                            if ($this->testa_situacao_aprovada($prop->situacao)) {
                                array_push($array_propostas_por_ano['2016apro'], $prop->codigo_siconv);
                            }
                        }
                        break;
                    case '2017':
                        array_push($array_propostas_por_ano['2017'], $prop->codigo_siconv);
                        if ($this->testa_situacao_enviada($prop->situacao)) {
                            array_push($array_propostas_por_ano['2017envi'], $prop->codigo_siconv);
                            if ($this->testa_situacao_aprovada($prop->situacao)) {
                                array_push($array_propostas_por_ano['2017apro'], $prop->codigo_siconv);
                            }
                        }
                        break;
                }
        ...
        return $array_propostas_por_ano;
    }
    ...
    function get_all_ativo_cadastradas($id_usuario, $filtro = "") {
        ...
        $this->db->where('ativo', 1);
        $this->db->where('enviado is null');
        $this->db->where('padrao is null');
    ...
    public function check_proposta_aprovada($idProposta, $idusuario, $situacao) {
        ...
        if ($this->banco_proposta_model->verifica_proposta_aprovada($situacao)) {
            if ($dados_gestor->tipo_gestor == 10) {
                ...
                $msg = "Proposta Número: <b><a href='https://www.convenios.gov.br/siconv/ConsultarProposta/ResultadoDaConsultaDePropostaDetalharProposta.do?idProposta={$proposta->id_proposta_efetiva}&path=/MostraPrincipalConsultarPrograma.do&Usr=guest&Pwd=guest&destino=DetalharParecerProposta' target='_blank'>" . $proposta->id_siconv . "</a></b>";
    ...
    public function check_proposta_banco_proposta_aprovada($idProposta, $idusuario, $situacao) {
        ...
        if ($this->banco_proposta_model->verifica_proposta_aprovada($situacao)) {
            if ($dados_gestor->tipo_gestor == 10) {
                ...
                $msg = "Proposta Número: <b><a href='https://www.convenios.gov.br/siconv/ConsultarProposta/ResultadoDaConsultaDePropostaDetalharProposta.do?idProposta={$proposta->id_siconv}&path=/MostraPrincipalConsultarPrograma.do&Usr=guest&Pwd=guest&destino=DetalharParecerProposta' target='_blank'>" . $proposta->codigo_siconv . "</a></b>";
                ...
                foreach ($admins as $user_admin) {
                    $this->envia_email($origem, $admin->email, null, $assunto, $mensagem);
                }

                //enviar para tecnicos
                $tecnicos = $this->gestor->get_tecnicos($this->gestor->get_all_by_usuario($usuario->id_usuario)->id_gestor);

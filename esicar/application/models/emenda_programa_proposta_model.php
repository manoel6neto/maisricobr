...
class emenda_programa_proposta_model extends CI_Model {
        ...
        if ($query->num_rows > 0) {
            ...
            if ($valor_total != null && $valor_total > 0) {
                ...
                if (count($propostas) > 0) {
                    $array_programas_usados_propostas = array();
                    foreach ($propostas as $prop) {
                        if ($prop->proponente == $beneficiario) {
                            if (count($this->programa_proposta_model->get_programas_by_proposta_programa($prop->idProposta, $programa)) > 0) {
                                $array_programas_usados_propostas = array_merge($array_programas_usados_propostas, $this->programa_proposta_model->get_programas_by_proposta_programa($prop->idProposta, $programa));
                            }
                        }
                    }
                ...
                if (count($propostas_banco_propostas) > 0) {
                    ...
                    foreach ($propostas_banco_propostas as $prop_banco) {
                        if (count($this->programa_banco_proposta_model->get_by_id_proposta_programa($prop_banco->id_proposta, $programa)) > 0) {
                            $array_programas_banco_proposta = array_merge($array_programas_banco_proposta, $this->programa_banco_proposta_model->get_by_id_proposta_programa($prop_banco->id_proposta, $programa));
                        }
                    }
    ...
    public function get_emendas_by_programa($codigo_programa, $tipo) {
        if ($tipo == 'PARLAMENTAR') {
            $this->db->select('codigo_programa, cnpj, nome, valor, emenda, parlamentar, data_inicio_benef, data_fim_benef, data_inicio_parlam, data_fim_parlam');
            $this->db->where("emenda !=''");
        }elseif($tipo == 'ESPECIFICO'){
            $this->db->select('codigo_programa, cnpj, nome, valor, data_inicio_benef, data_fim_benef, data_inicio_parlam, data_fim_parlam');
            $this->db->where("emenda", '');
        }
        $this->db->where('codigo_programa', $codigo_programa);
        return $this->db->get('siconv_beneficiario')->result_array();
    }

    public function update_excluido($emenda) {
        $this->db->where('codigo_programa', $emenda['codigo_programa']);
        $this->db->where('cnpj', $emenda['cnpj']);
        $this->db->where('nome', $emenda['nome']);
        $this->db->where('emenda', $emenda['emenda']);
        $this->db->where('parlamentar', $emenda['parlamentar']);
        $this->db->update('siconv_beneficiario', array('excluido' => 1));
    }

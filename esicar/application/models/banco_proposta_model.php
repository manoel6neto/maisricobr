...
class banco_proposta_model extends CI_Model {
    ...
    function get_propostas_by_proponentes_anos($cnpjs, $anos) {
        $this->db->where_in('proponente', $cnpjs);
        $this->db->where_in('ano', $anos);
        $query = $this->db->get('banco_proposta');
    ...
    /**
     * Determina se um programa vigente possui propostas
     * 
     * @param type $programa - Um código de programa
     * @param type $proponentes - Códigos (CNPJ) dos proponentes
     * @return type - True se o programa tiver alguma proposta. False caso contrário.
     */
    function check_programa_tem_propostas($programa, $proponentes) {

        $this->db->where('codigo_programa', $programa);
        if (!empty($proponentes))
            $this->db->where_in('proponente', $proponentes);
    ...
    public function get_propostas_proponente_programa($cnpjs, $programa) {
        if (is_array($programa)) {
            $codigos_programas = array();
            foreach ($programa as $p) {
                $codigos_programas[] = $p->codigo;
            }
            $this->db->where_in('codigo_programa', $codigos_programas);
        } else {
            $this->db->where('codigo_programa', $programa);
        }
    ...
    function insert_or_update($options, $onlyadd = false) {
        $this->db->where('codigo_siconv', $options['codigo_siconv']);
        //$this->db->where("id_siconv <= {$options['id_siconv']}");
        $query = $this->db->get('banco_proposta');

        if ($query->num_rows > 0) {
            if ($query->row(0)->id_siconv <= $options['id_siconv']) {
                if (!$onlyadd) {
                    $this->db->where('id_proposta', $query->row(0)->id_proposta);
                    $this->db->update('banco_proposta', $options);
                    return $query->row(0)->id_proposta;
                } else {
                    //get all proponentes clientes
                    $proponentes = array();
                    $this->load->model('cnpj_siconv');
                    $proponentes_object = $this->cnpj_siconv->get_proponentes_from_gestores_ativos();
                    foreach ($proponentes_object as $proponente) {
                        if (!in_array($proponente->cnpj, $proponentes)) {
                            array_push($proponentes, $proponente->cnpj);
                        }
                    }
                    if (!in_array($options['proponente'], $proponentes)) {
                        $this->db->where('id_proposta', $query->row(0)->id_proposta);
                        $this->db->update('banco_proposta', $options);
                        return $query->row(0)->id_proposta;
                    }
    ...
    public function busca_programas_pareceres($filtros, $busca_cnpj_proponente = false, $filtro_proponente = array()) {
        ...
        if ((!empty($cnpjs) && $this->session->userdata('nivel') != 1) || $this->session->userdata('nivel') == 1) {
            ...
            $this->db->where_not_in('UPPER(situacao)', array(strtoupper('Proposta/Plano de Trabalho em rascunho'), strtoupper('Histórico'), strtoupper('Historico')));
    ...
    public function busca_programas_objeto($filtros, $busca_cnpj_proponente = false, $filtro_proponente = array()) {
        $this->load->model('usuariomodel');
        $this->load->model('programa_model');
        ...
        if ((!empty($cnpjs) && $this->session->userdata('nivel') != 1) || $this->session->userdata('nivel') == 1) {
            ...
            if ($this->session->userdata('nivel') == 12 || $this->session->userdata('nivel') == 13 || $this->session->userdata('nivel') == 14) {
                ...
                if (isset($filtros['anos']) && count($filtros['anos']) > 0) {
                    $this->db->where_in('ano', $filtros['anos']);
                } else
                    return null;
                ...
                if (isset($filtros['status_prop']) && count($filtros['status_prop']) > 0) {
                    if (count($filtros['status_prop']) == 2) {
                        $this->db->where_in('situacao', array_merge($this->retorna_status_aprovado(), $this->retorna_status_analise()));
                    } elseif ($filtros['status_prop'][0] == 1) {
                        $this->db->where_in('situacao', $this->retorna_status_aprovado());
                    } elseif ($filtros['status_prop'][0] == 2) {
                        $this->db->where_in('situacao', $this->retorna_status_analise());
                    }
                } else
                    return null;
                ...
                if (isset($filtros['proponente']) && count($filtros['proponente']) > 0) {
                    foreach ($filtros['proponente'] as $proponente)
                        $listaCnpjs[] = preg_replace('/[^0-9]/', '', (string) $proponente);
                    $this->db->where_in('proponente', $listaCnpjs);
                }
    ...
    public function verifica_proposta_analise($situacao) {
        return in_array(strtoupper($situacao), $this->retorna_status_analise());
    }
    ...
    public function retorna_status_analise() {
        return array(

...
class proponente_siconv_model extends CI_Model {
    ...
    public function getListaEstados() {
        ...
        if ($this->session->userdata('nivel') == 13 || $this->session->userdata('nivel') == 14) {
            ...
            $estados_bloquear = $this->estados_direito_gestor_execucao_model->get_lista_estados_bloqueados($this->session->userdata('id_usuario'));
    ...
    public function getListaEsferas() {
        ...
        if ($this->session->userdata('nivel') == 4 || $this->session->userdata('nivel') == 15) {
            ...
            if (key_exists("PRIVADA", $listaEsferas)) {
                $listaEsferas["PRIVADA"] = "Organização da Sociedade Civil";
            }
            if (key_exists("MUNICIPAL", $listaEsferas)) {
                $listaEsferas["MUNICIPAL"] = "Governo Municipal";
            }
            if (key_exists("ESTADUAL", $listaEsferas)) {
                $listaEsferas["ESTADUAL"] = "Governo Estadual";
            }
            if (key_exists("CONSORCIO PUBLICO", $listaEsferas)) {
                $listaEsferas["CONSORCIO PUBLICO"] = "Consórcio Público";
            }
            if (key_exists("EMPRESA PUBLICA SOCIEDADE ECONOMIA MISTA", $listaEsferas)) {
                $listaEsferas["EMPRESA PUBLICA SOCIEDADE ECONOMIA MISTA"] = "Empresas Públicas/Economia Mista";
            }
            ...
            switch ($this->session->userdata('sistema')) {
                case "M":
                    return array("MUNICIPAL" => "Governo Municipal",
                        "ESTADUAL" => "Governo Estadual",
                        "CONSORCIO PUBLICO" => "Consórcio Público",
                        "EMPRESA PUBLICA SOCIEDADE ECONOMIA MISTA" => "Empresas Públicas/Economia Mista",
                        "PRIVADA" => "Organização da Sociedade Civil"
                    );
                    break;
                case "E":
                    return array("ESTADUAL" => "Governo Estadual",
...
// 			$this->db->join('proponente_direito_vendedor', "proponente = cnpj and id_vendedor = {$this->session->userdata('id_usuario')}");
    ...
    public function get_municipio_estados($uf) {
        $this->db->_protect_identifiers = false;
        ...
        $this->db->distinct();
        $this->db->select("codigo_municipio, municipio");
        $this->db->where_in('municipio_uf_sigla', $uf);
        $this->db->order_by("municipio", "ASC");

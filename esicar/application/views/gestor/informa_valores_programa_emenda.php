...
//         }
        ...
        var valida_global = parseFloat(replaceAll(document.getElementById('salvarValorContrapartida').value, '.','').replace(",", ".")) + parseFloat(replaceAll(document.getElementById('salvarValorRepassevoluntario').value, '.','').replace(",", "."));
...
<div class="innerALl">
	...
	<h3 style="color: #428bca;">Selecionar Objetos / Preencher Valores</h3>
			...
			<form name="" id="formprop" method="post" action="incluir_proposta" enctype="multipart/form-data">
                ...
                <?php 
                unset($valores_programa['valores_emendas']);
                if(isset($valores_programa)){
                	foreach ($valores_programa as $valor){
                        ...
                        <?php
                        if (isset($emendas_usadas) !== false) {
                        	foreach($emendas_usadas as $emenda_parlamentar) {
                            	$emenda_parlamentar = (object)$emenda_parlamentar;
                                if ((isset($_GET['edit']) && $emenda_parlamentar->id_programa_proposta == $valor->id_programa_proposta) || (isset($emenda_parlamentar->numero_programa) && $emenda_parlamentar->numero_programa == $valor->codigo_programa)) {
                        ?>
                        			<input class="emendas_hidden" type="hidden" name="valorEmenda<?php echo $emenda_parlamentar->numero_emenda; ?>" value="<?php echo number_format($emenda_parlamentar->valor_utilizado, 2, ",", "."); ?>">
                        <?php
                            	}
                        	}
                        }
...
<div id="preenche_dados_programa" title="Valores do Programa" class="bg-white">
	<form action="#" id="form_modal">
		...
		<div class="col-md-8 bg-white">
			...
			<?php if (isset($emendas_disponiveis) !== false): ?>
                                    <?php 
                                        foreach($emendas_disponiveis as $emenda_parlamentar) {
                                        ?>    
                                            <div class="form-group">
                                                <label><?php echo $emenda_parlamentar->parlamentar . " - " . $emenda_parlamentar->emenda . " - R$ "; ?> <span style="color: red"><?php echo number_format($emenda_programa_proposta_model->get_valor_disponivel_emenda_from_emenda_beneficiario($codigo_programa, $emenda_parlamentar->emenda, $cnpjProponente, $usuario_logado), 2, ",", "."); ?></span></label> <input type="text"
						tabindex="1" name="valorEmenda_<?php echo $emenda_parlamentar->emenda; ?>_modal" onmouseup="verifica()" maxlength="14"
                                                onkeyup="verifica()"
						id="salvarValorEmenda<?php echo $emenda_parlamentar->emenda; ?>"
						onkeypress="reais(this,event)"
						value="" class="form-control campoNumerico numericoRequired valoresEmendas">
                                            </div>
                                    <?php
                                        }
                                        ?>
...
<script type="text/javascript" language="Javascript1.1"
accesskey=""src="<?php echo  base_url();?>configuracoes/js/preenchimento_valores_proposta.js"></script>
...
$(document).ready(function(){
		...
		buttons: {
			"Confirmar":function(){
				...
				if(enviardadosProposta(document.getElementById("form_modal"))){
					...
					var total_emendas = 0;
                    $('#form_modal .valoresEmendas').each(function(){
                    	var name = $(this).attr("name").split("_");
                        campos += '<input class="emendas_hidden" type="hidden" name="valorEmenda'+name[1]+'" value="'+$(this).val()+'">';
                        total_emendas += parseFloat(replaceAll($(this).val(), '.','').replace(",", "."));
					});

                    if(isNaN(total_emendas)){
                        //alert("Informe o valor para a(s) emenda(s)");
                        //return false;
                    }
                    
                    if (total_emendas < parseFloat(replaceAll($("input[name='valorRepasse_modal']").val(), '.','').replace(",", "."))) {
                        //alert("Valor das emendas inferior ao valor de repasse");
                        //return false;
                    } else if (total_emendas > parseFloat(replaceAll($("input[name='valorRepasse_modal']").val(), '.','').replace(",", "."))) {
                    	//alert("Valor das emendas superior ao valor de repasse");
                        //return false;
                    }
	...
	function carrega_campos_modal(idPrograma){
		if($("#"+idPrograma+"_modal").length){
			...
			$('.emendas_hidden').each(function(){
				var name = $(this).attr("name").split("valorEmenda");
                $("#form_modal input[name='valorEmenda_"+name[1]+"_modal']").val($(this).val());
			});
		}
	}

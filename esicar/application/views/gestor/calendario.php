        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-success" onclick="nova_tarefa()" id="salvar">Salvar</button>
...
<?php foreach ($eventos as $value) : ?>
        ...
        data.push({id: <?= $value->id ?>, title: '<?= $value->titulo ?>', start: new Date(<?= explode('-', $value->data_inicio)[0] ?>, <?= explode('-', $value->data_inicio)[1] ?> - 1, <?= explode('-', $value->data_inicio)[2] ?>), end: end, text: '<?= $value->descricao ?>', situacao: '<?= $value->situacao ?>', existente: '<?= $value->existente ?>'});
<?php endforeach; ?>
//Ordena eventos
...
//Atualiza o calendário
    ...
    function finalizar(id) {
        $.ajax({
            url: '<?php echo base_url('index.php/calendario/finalizar') ?>',
            dataType: 'html',
            type: 'post',
            data: {
                id: id
            ...
            success: function (data) {
                if (data === 'true') {
                    alert("O status da tarefa foi alterado com sucesso!");
                    location.reload();
                } else
                    alert("Não foi possivel alterar o status!");
    ...
    function editar(titulo, descricao, inicio, fim, id) {
        if (document.querySelector('#nova_tarefa')) {
            document.getElementById("nova_tarefa").reset();
            document.getElementById('nova_tarefa').id = 'editar_tarefa';
            document.getElementById("salvar").setAttribute("onClick", "salvar_edicao(" + id + ");");
        }
        $('#novaTarefa').modal('show');
    ...
    function excluir(id) {
        $.ajax({
            url: '<?php echo base_url('index.php/calendario/excluir') ?>',
            dataType: 'html',
            type: 'post',
            data: {
                id: id
            ...
            success: function (data) {
                if (data === 'true') {
                    alert("A tarefa foi excluída com sucesso!");
                    location.reload();
                } else
    ...
    function nova_tarefa() {
        $('#nova_tarefa').submit(function (e) {
            e.preventDefault();
            $.post("<?php echo base_url('index.php/calendario/nova_tarefa') ?>", {
                titulo: $("#titulo").val(),
        ...
        $("#nova_tarefa").validate({
            ...
            messages: {
                ...
                fim: {
                    required: "A data do fim do evento é obrigatória",
                    date: "Insira uma data valida"
                }
            }
        });

        if ($('#nova_tarefa').valid())
            $('#nova_tarefa').submit();
    }
    ...
    function salvar_edicao(id) {
        $('#editar_tarefa').submit(function (e) {
            e.preventDefault();
            $.post("<?php echo base_url('index.php/calendario/editar') ?>", {
                titulo: $("#titulo").val(),
                descricao: $("#descricao").val(),
                inicio: $("#inicio").val(),
                fim: $("#fim").val(),
                ...
                id: id
                    ...
                    function (data, status) {
                        if (data === "true") {
                            ...
                            alert("Não foi possível atualizar evento");
                            console.log(data);
                        }
                    });
        });
        ...
        if ($('#editar_tarefa').valid())
            $('#editar_tarefa').submit();
    }
</script>
